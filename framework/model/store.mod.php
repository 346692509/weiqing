...
function store_goods_info($id) {
	$result = table('store_goods')->getById(intval($id));
...
function store_order_info($id) {
	$result = table('store_order')->getById(intval($id));
	return $result ? $result : array();
}
...
function store_get_cash_orders($condition = array(), $page = 1, $psize = 15) {
	...
	$goods = table('store_goods')->where('id', $goods_ids)->getall('id');
	$orders = table('store_order')->where('id', $order_ids)->getall('id');
	foreach ($cash_orders as $k => $order) {
