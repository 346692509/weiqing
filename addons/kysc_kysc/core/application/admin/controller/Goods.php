...
class Goods extends BaseController
{
    ...
    public function addGoods()
    {
        ...
        if ($goodsId > 0) {
            ...
            if (! empty($goods_info)) {
                ...
                $virtual_goods_count = $virtual_goods->getVirtualGoodsCountByGoodsId($goodsId);
    ...
    public function virtualGoodsList()
    {
        ...
        $virtual_group_info = $virtual_goods->getVirtualGoodsGroupInfo($virtual_goods_group_id);
    ...
    public function updateGoodsCategory()
    {
        ...
        if (request()->isAjax()) {
            ...
            $result = $goodscate->addOrEditGoodsCategory($category_id, $category_name, $short_name, $pid, $is_visible, $keywords, $description, $sort, $category_pic, $attr_id, $attr_name, $pc_custom_template, $wap_custom_template, $levels);
    ...
    public function GoodsCreateOrUpdate()
    {
        ...
        if (! empty($product)) {
            ...
            $res = $goodservice->addOrEditGoods(
                ...
                $product['virtual_goods_type_data'], 
                $product['production_date'], 
                $product['shelf_life'], 
                $product['ladder_preference'], 
                $product['goods_video_address'], 
                $product['pc_custom_template'], 
                $product['wap_custom_template'], 
                $product['max_use_point'], 
                $product['is_open_presell'], 
                $product['presell_delivery_type'], 
                $product['presell_price'], 
                $product['presell_time'], 
                $product['presell_day'], 
                $product['goods_unit'],
    ...
    public function addGoodsGroup()
    {
        ...
        if (request()->isAjax()) {
            ...
            $result = $goodsgroup->addOrEditGoodsGroup(0, $shop_id, $group_name, $pid, $is_visible, $sort, $group_pic, $group_dec);
            return AjaxReturn($result);
        } else {
    ...
    public function updateGoodsGroup()
    {
        ...
        if (request()->isAjax()) {
            ...
            $result = $goodsgroup->addOrEditGoodsGroup($group_id, $shop_id, $group_name, $pid, $is_visible, $sort, $group_pic, $group_dec);
            return AjaxReturn($result);
        } else {
    ...
    public function addGoodsSpec()
    {
        ...
        if (request()->isAjax()) {
            ...
            $goods_id = 0;
            $res = $goods->addGoodsSpecService($this->instance_id, $spec_name, $show_type, $is_visible, $sort, $spec_value_str, $attr_id, $is_screen, $spec_des, $goods_id);
            return AjaxReturn($res);
        }
    ...
    public function updateGoodsSpec()
    {
        ...
        if (request()->isAjax()) {
            ...
            $is_screen = request()->post('is_screen', 0);
            $spec_des = request()->post('spec_des', 0);
            $res = $goods->updateGoodsSpecService($spec_id, $this->instance_id, $spec_name, $show_type, $is_visible, $sort, $spec_value_str, $is_screen, $spec_des);
            return AjaxReturn($res);
        }
    ...
    public function addAttributeService()
    {
        ...
        $goodsguige = $goods->getGoodsSpecList(1, 0, $condition, 'sort desc');
        ...
        if (request()->isAjax()) {
            ...
            $goodsAttribute = $goods->addAttributeService($attr_name, $is_use, $spec_id_array, $sort, $value_string, $brand_id_array);
            return AjaxReturn($goodsAttribute);
        }
        
        // 品牌
        $goodsbrand = new GoodsBrand();
        $brand_list = $goodsbrand->getGoodsBrandList();
    ...
    public function updateGoodsAttribute()
    {
        ...
        if (request()->isAjax()) {
            ...
            $value_string = request()->post('data_obj_str', '');
            $brand_id_array = request()->post('select_brank', '');
            $res = $goods->updateAttributeService($attr_id, $attr_name, $is_use, $spec_id_array, $sort, $value_string, $brand_id_array);
            return AjaxReturn($res);
        }
    ...
    public function getSelectGoodslist()
    {
        ...
        $list = $goods->getSelectGoodsList($page_index, 0, $condition, "sort asc,create_time desc", "goods_id,goods_name,stock,promotion_price,price");
        return $list;
    }
