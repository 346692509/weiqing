...
class Pintuan extends Order
{
        ...
        if (! empty($order_list['data'])) {
            foreach ($order_list['data'] as $k => $v) {
                ...
                $tuangou_group_info = $tuangou_group->getInfo([
                    "group_id" => $v["tuangou_group_id"]
                ], "status");
                if (empty($tuangou_group_info)) {
                    $tuangou_group_status = - 1;
                } else {
                    $tuangou_group_status = $tuangou_group_info["status"];
                }
                // 拼团
        ...
        try {
            ...
            foreach ($order_list as $k => $v) {
                ...
                foreach ($order_goods_list as $t => $m) {
                    $this->orderGoodsConfirmRefund($v["order_id"], $m["order_goods_id"], $v["pay_money"], $v["user_platform_money"], $v["payment_type"], '拼团失败后退款');
                }
                // 关闭订单
    ...
    public function tuangouOrderGoodsConfirmRefund(){
