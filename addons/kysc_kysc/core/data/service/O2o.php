...
use data\model\NsO2oDistributionUserModel;
...
class O2o extends BaseService
{
    ...
    */ 
     ...
     * 添加本地配送地区
     */
    public function addOrUpdateDistributionArea($store_id, $province_id, $city_id, $district_id)
    {
        Cache::clear("address");
        $o2oDistributionArea = new NsO2oDistributionAreaModel();
        $res = $this->getDistributionAreaInfo($store_id);
        if ($res == '') {
            $data = array(
                "store_id" => $store_id,
    ...
    public function O2oOrderDelivery($order_id, $order_delivery_user_id, $express_no, $remark){
        ...
        try {
            ...
            $order ->orderDoDelivery($order_id); //订单主表发货
