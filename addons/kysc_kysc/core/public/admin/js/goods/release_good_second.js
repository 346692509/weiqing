 ...
 * 修改时间：2018年4月27日15:25:19 Line：2567	全栈小学生
 * 更新日志：
 * 1、 [新增] 修改规格值功能
 * 2、 [新增] 删除规格、规格值功能
...
$(function() {
	...
	if(parseInt($("#goodsId").val()) > 0){
		...
		var sku_display_mode_html = "";
		var spec_length = $specObj.length;
...
//						sort : ($(".edit-sku-popup-body article>div.sku-value ul li").length-1),
						spec_id: curr_spec.value[j].spec_id,
						spec_value_data: curr_spec.value[j].spec_value_data,
						spec_value_id: curr_spec.value[j].spec_value_id,
						spec_value_name: curr_spec.value[j].spec_value_name
					});
					
					
					sku_display_mode_html += '<li data-spec-value-id="' + curr_spec.value[j].spec_value_id + '" data-spec-show-type="1" style="display:none;">';
					sku_display_mode_html += '<span>' + curr_spec.value[j].spec_value_name + '</span><strong>编辑</strong>';
					sku_display_mode_html += '</li>';
					
					if(curr_spec.value[j].spec_show_type == 2){
						sku_display_mode_html += '<li data-spec-value-id="' + curr_spec.value[j].spec_value_id + '" data-spec-show-type="2" class="selected" style="display:none;">';
					}else{
						sku_display_mode_html += '<li data-spec-value-id="' + curr_spec.value[j].spec_value_id + '" data-spec-show-type="2" style="display:none;">';
					}
						sku_display_mode_html += '<input type="color" class="input-common-color" data-spec-value-id="' + curr_spec.value[j].spec_value_id + '" value="' + (curr_spec.value[j].spec_value_data == "" ? "#000000" : curr_spec.value[j].spec_value_data) + '">';
					sku_display_mode_html += '</li>';
					
					if(curr_spec.value[j].spec_show_type == 3){
						sku_display_mode_html += '<li data-spec-value-id="' + curr_spec.value[j].spec_value_id + '" data-spec-show-type="3" class="selected" style="display:none;">';
					}else{
						sku_display_mode_html += '<li data-spec-value-id="' + curr_spec.value[j].spec_value_id + '" data-spec-show-type="3" style="display:none;">';
					}
						sku_display_mode_html += '<div class="upload-btn-common">';
							sku_display_mode_html += '<div>';
								// sku_display_mode_html += '<input class="input-file" name="file_upload" id="uploadImg' + curr_spec.value[j].spec_value_id + '" type="file" onchange="skuImgUpload(this);" title="上传" data-spec-value-id="' + curr_spec.value[j].spec_value_id + '">';
								sku_display_mode_html += '<input type="hidden" id="sku_value_' + curr_spec.value[j].spec_value_id + '" value="' + curr_spec.value[j].spec_value_data + '">';
								sku_display_mode_html += '</div>';
								
								//兼容以前的数据结构
								if(curr_spec.value[j].spec_value_data_src != null) curr_spec.value[j].spec_value_data = curr_spec.value[j].spec_value_data_src;
								
								if(curr_spec.value[j].spec_show_type == 3 && curr_spec.value[j].spec_value_data != ""){
									sku_display_mode_html += '<em>修改图片</em>';
									sku_display_mode_html += '<img id="preview_sku_value_' + curr_spec.value[j].spec_value_id + '" src="' + STATIC + '/blue/img/upload-common-select-blue.png" data-html="true" data-container="body" data-placement="top" data-trigger="manual" data-original-title="" title="" data-src="' + __IMG(curr_spec.value[j].spec_value_data) + '">';
								}else{
									sku_display_mode_html += '<em>添加图片</em>';
									sku_display_mode_html += '<img id="preview_sku_value_' + curr_spec.value[j].spec_value_id + '" src="' + STATIC + '/blue/img/upload-common-select.png" data-html="true" data-container="body" data-placement="top" data-trigger="manual" data-original-title="" title="">';
								}
						sku_display_mode_html += '</div>';
					sku_display_mode_html += '</li>';
				...
				sku_list_html += "<li data-spec-id="+ curr_spec.spec_id +" data-spec-value-json=" + JSON.stringify(values) + " data-spec-name='" + curr_spec.spec_name + "' data-show-type='" + curr_spec.value[0].spec_show_type + "' data-spec-value-length='" + curr_spec.value.length + "'><span>" + curr_spec.spec_name + "</span><span>[" + 0 + "/" + curr_spec.value.length + "]</span></li>";
			}

			//规格弹出框代码
			sku_list_html += '<li class="add-sku"><input type="text" class="input-common" placeholder="输入新规格名称"><i class="fa fa-plus"></i><strong>添加临时规格</strong></li>';
			
			$(".js-sku-list-popup").html(sku_list_html);
			$('.js-sku-list-popup li:not(.add-sku)').arrangeable();
			$(".edit-sku-popup-body article>div.sku-display-mode>ul").html(sku_display_mode_html);
			...
			$(".edit-sku-popup-body article>div.sku-display-mode nav ul li[data-spec-show-type='" + $specObj[0].value[0].spec_show_type + "']").addClass("selected").find(".radio-common").addClass("selected").children("input").attr("checked","checked");
			editSkuData();
			
			$(".js-sku-list-popup li:eq(0):not(.add-sku)").click();
		}else{
//			编辑商品时，如果该商品没有选择过规格，则加载公共规格列表
...
//			//如果没有选择商品类型，则清空属性信息
	...
	$(".goods-sku-item .js-goods-spec-value-img").live("mouseenter",function(){
		...
		if(curr.children("input[type='hidden']").val() != ""){
			...
			var contents = '<img src="'+src+'" style="max-width: 150px;height: auto;display:block;margin:0 auto;">';
	...
	$(".goods-sku-item").live("mouseleave",function(){
		...
		if(curr.children("input[type='hidden']").val() != "") curr.popover("hide");
	});
	...
	$('#img_box').live('click',function(e){		
		...
		OpenPricureDialog("PopPicure", ADMINMAIN, 0, 1,0,0,UPLOADGOODS);
	...
	$('#sku_img_box').live('click',function(e){
		...
		shopImageFlag = js_img;//所点击的商品图片标识
		speciFicationsFlag = 0;
		OpenPricureDialog("PopPicure", ADMINMAIN, 0, 2, spec_id, spec_value_id,UPLOADCOMMON);
	});
...
//		else  $(".h50").hide();
			...
			buttons: {
				"确定": function() {
					...
					$(".js-sku-list-popup li").each(function(){
						$(this).removeClass('selected curr').children("span:last").text("[0/" + $(this).attr("data-spec-value-length") + "]");
					});
					
					//清除规格图片
					removeSpecPictureBox();
					
					$(".edit-sku-popup-body article>div .sku-value-list").hide().siblings().show();

					$(".edit-sku-popup-body article .sku-display-mode ul li[data-spec-value-id]").remove();
	...
	$(".edit-sku-popup footer .btn-top").click(function(){
		...
		if($(".js-sku-list-popup li[data-spec-value-id-array]").length > 0){
			...
			$(".js-sku-list-popup li[data-spec-value-id-array]").each(function(i){
				...
				var is_update_img = parseInt($(this).attr("data-is-update-img"));
				...
				for(var i=0;i<spec_value.length;i++){
					...
					if($.inArray(parseInt(curr.spec_value_id),spec_value_id_array)>-1){
						...
						// if(spec_show_type == 3){
						// 	if(is_update_img != spec_value_id_array.length){
						// 		$(".edit-sku-popup-body aside>ul li[data-spec-id='" + curr.spec_id + "']").click();
						// 		showTip("请先上传图片","warning");
						// 		is_validation = false;
						// 		return false;
						// 	}
						// }
	...
	$(".edit-sku-popup-body article>div.sku-display-mode nav ul li").click(function(){
		...
		var curr_sku = $(".edit-sku-popup-body aside .current-selected-sku ul.js-sku-list-popup li.curr.selected");
		...
		if(show_type != select_show_type){
			$(".edit-sku-popup-body aside .current-selected-sku ul.js-sku-list-popup li.selected.curr").attr("data-show-type",select_show_type).click();
...
//			$(".edit-sku-popup-body article>div.sku-display-mode nav ul li[data-spec-show-type=2]").click();，颜色不影响，可以不用
	...
	/**
	 * 删除规格值
	 * 创建时间：2018年4月13日18:43:23 全栈小学生
	 */
	$(".edit-sku-popup-body article>div.sku-value ul li .shut").live("click",function(){
		
		var $this = $(this);
		if($(".js-sku-list-popup li.selected.curr").length == 0){
		...
		$( "#dialog").dialog({
			buttons: {
				"确定": function() {
					var curr_li = $(".js-sku-list-popup li.selected.curr");
...
$(".js-sku-list-popup li").live("click",function(){
	...
	$(this).addClass("selected curr").siblings(":not([data-spec-value-id-array])").removeClass("selected");
	...
	if(index != $(".js-sku-list-popup li").length-1){
		...
		$(".js-sku-list-popup li.add-sku").css({ "border-color" : "" }).find("input").hide().siblings().show();
		...
		//添加规格
		$(".edit-sku-popup-body article>div.sku-value .empty-info").text("输入新规格后，回车即可保存").show().siblings().hide();
		$(".js-sku-list-popup li.add-sku").css({ "border-color" : "#ff5a00" }).find("input").show().focus().siblings().hide();
...
$(".original-sku-list li").live("click",function(){
	...
	if(spec_value!=null){
		var spec_value_id_array = eval($(".js-sku-list-popup li[data-spec-id='" + spec_id + "']").attr("data-spec-value-id-array"));
		$(".edit-sku-popup-body article>div.sku-display-mode>ul li[data-spec-value-id]").hide();
		$(".edit-sku-popup-body article>div.sku-display-mode .empty-info").show().nextAll().hide();
		for(var i=0;i<spec_value.length;i++){
			var curr = spec_value[i];
...
$(".edit-sku-popup-body article>div.sku-value ul li:not(.add-sku-value)").live("click",function(){
	...
	// 判断该规格是否存在于当前规格区中
	var spec = $(".js-sku-list-popup li[data-spec-id='" + spec_id + "']");
	...
	if(!$(this).hasClass("selected")){
		...
		if(spec.length == 0 && curr_selected_num > 0 && spec_id > 0){
			$(".js-sku-list-popup li[data-spec-id]").removeClass("curr");
			...
			$(".js-sku-list-popup .add-sku").before(spec_html);
		...
		if($(".edit-sku-popup-body article>div.sku-display-mode>ul li[data-spec-value-id='" + spec_value_id +"']").length == 0){
			...
			if(spec_show_type == 3){
				...
				display_mode_html += '<div class="upload-btn-common">';
					display_mode_html += '<div>';
						// display_mode_html += '<input class="input-file" name="file_upload" id="uploadImg' + spec_value_id + '" type="file" onchange="skuImgUpload(this);" title="上传" data-spec-value-id="' + spec_value_id + '">';
			...
			if(spec_value_obj.attr("data-show-type") == 3 && spec_value_obj.attr("data-spec-value-data") != ""){
				var update_img_num = parseInt($(".js-sku-list-popup li[data-spec-id='" + $(this).attr("data-spec-id") + "']").attr("data-is-update-img")) + 1; 
				$(".js-sku-list-popup li[data-spec-id='" + $(this).attr("data-spec-id") + "']").attr("data-is-update-img", update_img_num);
			}
		}
	}else{
		//取消
		...
		var curr_selected_num = $(this).parent("ul").find(".selected").length;
		if(curr_selected_num == 0 && spec_id > 0){
			$(".js-sku-list-popup li[data-spec-id='" + spec_id + "']").remove();
			$(".edit-sku-popup-body article>div.sku-value ul li").remove();
		}
		...
		if($(this).attr("data-show-type") == 3 && $(this).attr("data-spec-value-data") != ""){
			var update_img_num = $(".js-sku-list-popup li[data-spec-id='" + $(this).attr("data-spec-id") + "']").attr("data-is-update-img"); 
	...
	if(spec_id_array.length){
		...
		$(".js-sku-list-popup li[data-spec-id='" + $(this).attr("data-spec-id") + "']").attr("data-spec-value-id-array",JSON.stringify(spec_id_array)).find("span:last").text("[" + spec_id_array.length + "/" + count + "]");
		$(".edit-sku-popup-body article>div.sku-display-mode .empty-info").hide().nextAll().show();
	}else{
		$(".js-sku-list-popup li[data-spec-id='" + $(this).attr("data-spec-id") + "']").removeAttr("data-spec-value-id-array").find("span:last").text("[" + spec_id_array.length + "/" + count + "]");;
		$(".edit-sku-popup-body article>div.sku-display-mode .empty-info").show().nextAll().hide();
	}
});
...
$(".js-sku-list-popup li.add-sku input").live("keyup",function(event){
	if(event.keyCode == 13){
		...
		//判断是否重复
		do{
			predefined_id = -(($(".js-sku-list-popup li").length-1) + Math.floor(new Date().getSeconds()) + Math.floor(new Date().getMilliseconds()));
			if($(".js-sku-list-popup li[data-spec-id='" + predefined_id + "']").length==0) break;
		...
		$('.js-sku-list-popup li:not(.add-sku)').arrangeable();
		$(".js-sku-list-popup li.add-sku").css({ "border-color" : "" }).find("input").hide().siblings().show();
		$(".js-sku-list-popup").animate({ "scrollTop" : $(".js-sku-list-popup")[0].scrollHeight },600);
		$this.val("");
		$(".js-sku-list-popup li:last").prev().click();
		showTip("添加规格成功","success");
	}
	// 按下esc
	if(event.keyCode == 27){
		$(".js-sku-list-popup li.add-sku").css({ "border-color" : "" }).removeClass('curr').find("input").hide().siblings().show();
		$(".js-sku-list-popup li:last").prev().click();
	}
});
...
/**
 * 添加规格值
 * 创建时间：2018年4月12日10:23:04 全栈小学生
 */
$(".edit-sku-popup-body article .add-sku-value-input input").live("keyup",function(event){
	if(event.keyCode == 13){
		var selected_spec = $(".edit-sku-popup-body aside .current-selected-sku ul.js-sku-list-popup li.curr.selected");
	...
	$.ajax({
		...
		success : function(res){	
			if(res !=-1){
				...
				$(".original-sku-list li[data-spec-id]").hide();
				
				if(spec_length>0){
					for(var i=0;i<spec_length;i++){
						var curr_spec = res.spec_list[i];

						if($(".original-sku-list li[data-spec-id='" + curr_spec.spec_id + "']").length == 0){
							//第一次加载
							sku_list_html += "<li title='"+ curr_spec.spec_des +"' data-spec-id="+ curr_spec.spec_id +" data-spec-value-json='" + JSON.stringify(curr_spec.values) + "' data-spec-name='" + curr_spec.spec_name + "' data-show-type='" + curr_spec.show_type + "' data-spec-value-length='" + curr_spec.values.length + "'><span>" + curr_spec.spec_name + "</span><span>[" + curr_spec.values.length + "]</span></li>";
						}else{
							//显示
							var curr_li = $(".original-sku-list li[data-spec-id="+ curr_spec.spec_id +"]");

							var spec_value = eval(curr_li.attr("data-spec-value-json"));
							//当前已选中规格值数量
							var spec_value_id_array = eval(curr_li.attr("data-spec-value-id-array"));
							
							var selectCount = 0;
							if(spec_value_id_array != null) selectCount = spec_value_id_array.length;

							//是否更新过规格值集合
							var is_update = false;
							// 公共规格值数组
							var curr_spec_value_array = new Array();
							for(var j=0;j<curr_spec.values.length;j++){
								curr_spec_value_array.push(curr_spec.values[j].spec_value_id);
							}
					
							//如果编辑了公共的规格值，只加载了当前商品的规格值，并没有加载公共的规格值，需要将公共的添加进来

							//当前的规格值
							for(var k=0;k<spec_value.length;k++){
								for(var j=0;j<curr_spec.values.length;j++){
									//找到当前已存在的规格值，合并
									if(spec_value[k].spec_value_id == curr_spec.values[j].spec_value_id){
										is_update = true;
										curr_spec.values[j].spec_value_name = spec_value[k].spec_value_name;
										break;
									}else{
										//将私有规格值进行合并
										if($.inArray(spec_value[k].spec_value_id,curr_spec_value_array) == -1) {
											curr_spec_value_array.push(spec_value[k].spec_value_id);
											curr_spec.values.push(spec_value[k]);
										}
									}
								}
							}
							...
							if(is_update){
								...
								$(".original-sku-list li[data-spec-id="+ curr_spec.spec_id +"]")
								.attr("data-spec-value-json",JSON.stringify(curr_spec.values))
								.attr("data-spec-value-length",curr_spec.values.length)
								.show()
								.children("span:last").text("[" + curr_spec.values.length + "]");
								// .children("span:last").text("[" + selectCount + "/" + curr_spec.values.length + "]");
							}else{
								$(".original-sku-list li[data-spec-id="+ curr_spec.spec_id +"]").show();
							}
						}
					}
					
					//记录新添加的规格
					var new_sku_list = $(".js-sku-list-popup li[data-spec-id^='-']");
					
					$(".original-sku-list .add-sku").before(sku_list_html);
					$(".js-sku-list-popup .add-sku").before(new_sku_list);
					$('.js-sku-list-popup li:not(.add-sku)').arrangeable();
				}
				
				//永远都显示新增的规格
				$(".js-sku-list-popup li[data-spec-id^='-']").show();
...
function getAttrSearchSelected(obj){
	...
	if(attr_id == -1){
		//编辑商品时，显示当前商品选择的规格
		var spec_length = $specObj.length;
		$(".js-sku-list-popup li[data-spec-id]").hide();
		if(spec_length>0){
			for(var i=0;i<spec_length;i++){
				var curr_spec = $specObj[i];
				var values = new Array();
				...
				$(".js-sku-list-popup li[data-spec-id="+ $specObj[i].spec_id +"]")
				.attr("data-spec-value-json",JSON.stringify(values))
				.attr("data-spec-value-length",values.length)
				.show()
				.children("span:last").text("[" + values.length + "/" + values.length + "]");
			}
			editSkuData();
		}
...
//规格图片
function skuImgUpload(event){

	$(".edit-sku-popup-body article>div.sku-display-mode nav ul li[data-spec-show-type=3]").click();
	
	var spec_value_id = $(event).attr("data-spec-value-id");
	var curr_sku = $(".edit-sku-popup-body aside .current-selected-sku ul.js-sku-list-popup li.selected.curr");
	var spec_value = eval(curr_sku.attr("data-spec-value-json"));
	var is_update_img = parseInt(curr_sku.attr("data-is-update-img"));
	
	var fileid = $(event).attr("id");
	var data = { 'file_path' : UPLOADGOODSSKU,"goods_path" : "add_goods" };
	var id = $(event).next().attr("id");
	var em = $(event).parent().next();
	
	if(parseInt(curr_sku.attr("data-show-type")) != 3){
		showTip("请先选中图片展示方式，再进行操作","warning");
		return;
	}
	...
	uploadFile(fileid,data,function(res){
		if(res.code){
			$("#"+id).val(res.data);
			$("#preview_" + id).attr("src",STATIC + "/blue/img/upload-common-select-blue.png").attr("data-src",__IMG(res.data));
			em.text("修改图片");
			if(spec_value != null){
				var is_auto = true;
				for(var i=0;i<spec_value.length;i++){
					if(spec_value_id == spec_value[i].spec_value_id){
						//如果是修改图片则不累计数量
						// if(spec_value[i].spec_value_data != "") is_auto = false;
						if(spec_value[i].spec_value_data.indexOf(UPLOADGOODSSKU) > -1) is_auto = false;						
						spec_value[i].spec_value_data = res.data;
					}
				}
				if(is_auto) ++is_update_img;
				curr_sku.attr("data-is-update-img",is_update_img).attr("data-spec-value-json",JSON.stringify(spec_value)).click();
			}
...
//规格图片
function skuPreviewUploadImg(event){
	// 上传图片时先清除之前的图片显示事件
	$(event).parent(".js-goods-spec-value-img").popover("destroy");

	$(".edit-sku-popup-body article>div.sku-display-mode nav ul li[data-spec-show-type=3]").click();
	
	var spec_value_id = $(event).attr("data-spec-value-id");
	var spec_id = $(event).attr("data-spec-id");
	var curr_sku = $(".edit-sku-popup-body aside .current-selected-sku ul.js-sku-list-popup li[data-spec-id="+spec_id+"]");
	var spec_value = eval(curr_sku.attr("data-spec-value-json"));
	var is_update_img = parseInt(curr_sku.attr("data-is-update-img"));
	
	var fileid = $(event).attr("id");
	var data = { 'file_path' : UPLOADGOODSSKU,"goods_path" : "add_goods" };
	var id = $("#uploadImg"+spec_value_id).next().attr("id");
	var em = $("#uploadImg"+spec_value_id).parent().next();
	...
	uploadFile(fileid,data,function(res){
		if(res.code){
			...
			$("#preview_" + id).attr("src",STATIC + "/blue/img/upload-common-select-blue.png").attr("data-src",__IMG(res.data));
			em.text("修改图片");			
			$("#"+fileid).prev("img").attr("src",__IMG(res.data));
			if(spec_value != null){
				var is_auto = true;
				for(var i=0;i<spec_value.length;i++){
					if(spec_value_id == spec_value[i].spec_value_id){
...
function againGenerateSkuJson(event){	
	...
	$(".js-sku-list-popup li[data-spec-id='" + spec_id + "']").attr("data-spec-value-json", JSON.stringify(spec_value_arr));
}
...
$('.sku-img-check').live('click',function(e){
	...
	shopImageFlag = js_img;//所点击的商品图片标识
	speciFicationsFlag = 0;
	OpenPricureDialog("PopPicure", ADMINMAIN, 1, 3,spec_id, spec_value_id, UPLOADGOODSSKU);
});

$(".upload-btn-common").live('click', function(){
	$(".edit-sku-popup-body article>div.sku-display-mode nav ul li[data-spec-show-type=3]").click();
	var spec_id = $(".edit-sku-popup-body .js-sku-list-popup li.curr").attr('data-spec-id');
	var spec_value_id = $(this).parent('li').attr('data-spec-value-id');
	OpenPricureDialog("PopPicure", ADMINMAIN, 1, 3,spec_id, spec_value_id, UPLOADGOODSSKU);
})