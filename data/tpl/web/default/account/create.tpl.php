...
<div id="js-account-create" ng-controller="commonCreateAccountCtrl" ng-cloak>
	<ol class="breadcrumb we7-breadcrumb">
		...
		<li>新建<?php  echo $account_all_type_sign[$sign]['title'];?></li>
	</ol>
	<ul class="we7-step">
		<li ng-class="step == 'base_info' ? 'active' : ''"> <span class="content">1. 设置<?php  echo $account_all_type_sign[$sign]['title'];?>信息</span> </li>
		<?php  if(!empty($_W['isfounder'])) { ?>
		<li ng-class="step == 'account_modules' ? 'active' : ''"> <span class="content">2. 设置权限</span></li>
		<?php  } ?>
		<?php  if(in_array($sign, array(ACCOUNT_TYPE_SIGN, XZAPP_TYPE_SIGN))) { ?>
		<li ng-class="step == 'account_help' ? 'active' : ''"><span class="content"><?php  if(empty($_W['isfounder'])) { ?>2<?php  } else { ?>3<?php  } ?> 引导页面</span></li>
		<?php  } else if(in_array($sign, array(WXAPP_TYPE_SIGN, PHONEAPP_TYPE_SIGN, ALIAPP_TYPE_SIGN, BAIDUAPP_TYPE_SIGN, TOUTIAOAPP_TYPE_SIGN))) { ?>
		<li ng-class="step == 'account_version' ? 'active' : ''"><span class="content"><?php  if(empty($_W['isfounder'])) { ?>2<?php  } else { ?>3<?php  } ?> 新建版本</span></li>
		<?php  } ?>
	</ul>

	<div ng-show="step == 'base_info'">
		<div class="we7-form">
			<?php  if($sign == ACCOUNT_TYPE_SIGN) { ?>
			...
			<?php  } else if($sign == WEBAPP_TYPE_SIGN) { ?>
			...
			<?php  } else if($sign == PHONEAPP_TYPE_SIGN) { ?>
			...
			<?php  } else if($sign == XZAPP_TYPE_SIGN) { ?>
			<div id="xzapp_base_info">
				...
				<div class="form-group">
					<label class="control-label col-sm-2">Oauth 2.0</label>
					<div class="form-controls col-sm-8">
						<p class="form-control-static">在熊掌号请求用户网页授权之前，开发者需要先到熊掌号后台的【开发设置】<b>基本设置</b>中配置授权回调域名。<?php  if($_W['isfounder']) { ?><?php  } ?></p>
					</div>
				</div>
				<div class="form-group">
					<label for="" class="control-label col-sm-2">头像</label>
			...
			<?php  } else if(in_array($sign, array(WXAPP_TYPE_SIGN, ALIAPP_TYPE_SIGN, BAIDUAPP_TYPE_SIGN, TOUTIAOAPP_TYPE_SIGN))) { ?>
			<div id="miniapp_base_info">
				<div class="form-defalut we7-form">
					<div class="form-group">
						<label for="" class="control-label col-sm-2"><?php  echo $sign_title;?>名称</label>
						<div class="form-controls col-sm-10">
							<input type="text" name="name" ng-model="account.name" class="form-control wxapp-name" placeholder="<?php  echo $account_all_type[$type]['title'];?>名称" required>
						</div>
					</div>
					<div class="form-group">
						<label for="" class="control-label col-sm-2"><?php  echo $sign_title;?>描述</label>
						<div class="form-controls col-sm-10">
					...
					<div class="form-group" ng-if="account.sign == 'baiduapp' || account.sign == 'toutiaoapp'">
	...
	<div ng-show="step == 'account_modules'">
		<div class="we7-form account-create-step">
			...
			<div class="col-sm-offset-2 form-controls col-sm-8" ng-show="groupAll.groups">
				...
				<div class="we7-header"><?php  echo $sign_title;?>附加应用权限组</div>
				...
				<div class="we7-header"><?php  echo $sign_title;?>附加应用</div>
			...
			<div class="col-sm-offset-2">
				...
				<input type="button" ng-click="submitAccountModules()" value="<?php  if(in_array($sign, array(WEBAPP_TYPE_SIGN))) { ?>完成<?php  } else { ?>下一步<?php  } ?>" class="btn btn-primary btn-submit"/>
			</div>
		</div>
	</div>
...
<script type="text/javascript">
	angular.module('accountApp').value('config', {
		...
		sign: "<?php  echo $sign;?>",
		sign_title: "<?php  echo $sign_title;?>",
		uid: "<?php  echo $_W['uid'];?>",
