...
<div id="js-account-manage-base" ng-controller="AccountManageBase" ng-cloak>
	...
	<?php  } else if($account->typeSign == XZAPP_TYPE_SIGN) { ?>
	<div class="alert alert-info" ng-if="account.isconnect == 0">
		<p><i class="wi wi-info-sign"></i>接入状态：未接入。</p>
		<p><i class="wi wi-info-sign"></i>解决方案：进入熊掌号后台，依次选择: 开发-> 开发设置-> 修改配置，然后将对应熊掌号在平台的url和token复制到熊掌号后台对应的选项，熊掌号后台会自动进行检测</p>
	</div>
	<?php  } ?>
	<table class="table we7-table table-hover table-form">
		...
		<?php  if(in_array($account->typeSign, array(ACCOUNT_TYPE_SIGN, WXAPP_TYPE_SIGN, PHONEAPP_TYPE_SIGN, ALIAPP_TYPE_SIGN, BAIDUAPP_TYPE_SIGN, TOUTIAOAPP_TYPE_SIGN))) { ?>
		<tr>
		...
		<?php  if(in_array($account->typeSign, array(ACCOUNT_TYPE_SIGN, WXAPP_TYPE_SIGN))) { ?>
		<tr>
		...
		<?php  if(in_array($account->typeSign, array(ACCOUNT_TYPE_SIGN, WXAPP_TYPE_SIGN, XZAPP_TYPE_SIGN))) { ?>
		<tr>
		...
		<?php  } else if($account_param->typeSign == WXAPP_TYPE_SIGN) { ?>
		<tr>
			<td class="table-label">接入方式</td>
		...
		<?php  } else if($account_param->typeSign == XZAPP_TYPE_SIGN) { ?>
		<tr>
			<td class="table-label">类型</td>
			<td ng-if="account.level == 1">个人</td>
			<td ng-if="account.level == 2">媒体</td>
			<td ng-if="account.level == 3">企业</td>
			<td ng-if="account.level == 4">政府</td>
			<td ng-if="account.level == 5">其他组织</td>
			<td class="text-right"><div class="link-group"><a href="javascript:;" data-toggle="modal" data-target="#level" ng-click="editInfo('level', account.level)">修改</a></div></td>
		</tr>
		<tr>
			<td class="table-label">接入方式</td>
			<td ng-if="account.type == 9">普通接入</td>
			<td ng-if="account.type == 10">授权接入</td>
			<td class="text-right" >
				<div class="link-group" ng-if="authstate && authurl">
					<a href="javascript:;" data-toggle="modal" data-target="#jointype" ng-click="editInfo('type', account.type)">修改</a>
				</div>
	...
	<?php  if($account->typeSign == ALIAPP_TYPE_SIGN) { ?>
	...
	<div class="modal fade" id="level" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="we7-modal-dialog modal-dialog we7-form">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
					<div class="modal-title">修改熊掌号类型</div>
				</div>
				<div class="modal-body">
					<div class="form-group">
						<select class="we7-select" ng-model="middleAccount.level">
							<option value="1" ng-selected="middleAccount.level == 1">个人</option>
							<option value="2" ng-selected="middleAccount.level == 2">媒体</option>
							<option value="3" ng-selected="middleAccount.level == 3">企业</option>
							<option value="4" ng-selected="middleAccount.level == 4">政府</option>
							<option value="4" ng-selected="middleAccount.level == 5">其他组织</option>
						</select>
					</div>
				</div>
				<div class="modal-footer">
	...
	<?php  } ?>
	<?php  if($account->typeSign == XZAPP_TYPE_SIGN) { ?>
		...
		<div class="we7-modal-dialog modal-dialog we7-form">
			<div class="modal-content">
				...
				<div class="modal-body text-center">
					<input id="type-1" type="radio" name="jointype" value="{{xzapp_normal}}" ng-model="middleAccount.type" ng-checked="middleAccount.type == xzapp_normal">
					<label class="radio-inline" for="type-1">普通接入</label>
					<input id="type-2" type="radio" name="jointype" value="{{xzapp_auth}}" ng-model="middleAccount.type" ng-checked="middleAccount.type == xzapp_auth">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="form-group" ng-show="middleAccount.endtype == 2">
						...
						<div class="help-block" ng-if="!founder">
							平台有效期不可超过主管理员有效期的最大期限（主管理员到期：<?php  echo $user_endtime;?>）
						</div>
					</div>
				</div>
				<div class="modal-footer">
		...
		<div class="we7-modal-dialog modal-dialog we7-form">
			<div class="modal-content">
				...
				<div class="modal-body text-center">
					...
					<label class="radio-inline" for="type-2">授权接入</label>
					<?php  } else if($account->typeSign == WXAPP_TYPE_SIGN) { ?>
	...
	<?php  } ?>
	<?php  if(in_array($account->typeSign, array(ACCOUNT_TYPE_SIGN, WXAPP_TYPE_SIGN, XZAPP_TYPE_SIGN, BAIDUAPP_TYPE_SIGN, TOUTIAOAPP_TYPE_SIGN))) { ?>
	<table class="table we7-table table-hover <?php  if($account->typeSign == WXAPP_TYPE_SIGN) { ?>table-form wechat-menu<?php  } else if($account->typeSign == BAIDUAPP_TYPE_SIGN) { ?>table-form<?php  } ?>">
		<col width="140px " />
		<col />
		<col width="100px" />
		<tr><th class="text-left" colspan="3"><?php  if(in_array($account->typeSign, array(ACCOUNT_TYPE_SIGN, BAIDUAPP_TYPE_SIGN, TOUTIAOAPP_TYPE_SIGN))) { ?>自定义菜单通讯设置<?php  } else { ?>开发者ID<?php  } ?></th></tr>
		<tr>
			<td class="table-label"><?php  if($account->typeSign == XZAPP_TYPE_SIGN) { ?>ClientId<?php  } else if(in_array($account->typeSign, array(BAIDUAPP_TYPE_SIGN, TOUTIAOAPP_TYPE_SIGN))) { ?>AppKey<?php  } else { ?>AppId<?php  } ?></td>
			<td ng-bind="account.key"></td>
			<td><div class="link-group"><a href="javascript:;" data-toggle="modal" data-target="#key"  ng-click="editInfo('key', account.key)">修改</a></div></td>
		</tr>
		<tr>
			<td class="table-label"><?php  if($account->typeSign == XZAPP_TYPE_SIGN) { ?>ClientSecret<?php  } else { ?>AppSecret<?php  } ?></td>
	...
	<div class="modal fade" id="key" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="we7-modal-dialog modal-dialog we7-form">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
					<div class="modal-title">修改<?php  echo $account->typeName?><?php  if($account->typeSign == XZAPP_TYPE_SIGN) { ?>ClientId<?php  } else if($account->typeSign == BAIDUAPP_TYPE_SIGN) { ?>AppKey<?php  } else { ?>AppId<?php  } ?></div>
				</div>
				<div class="modal-body">
					<div class="form-group">
	...
	<?php  } ?>
	<?php  if(in_array($account->typeSign, array(BAIDUAPP_TYPE_SIGN, TOUTIAOAPP_TYPE_SIGN))) { ?>
	<table class="table we7-table table-hover table-form wechat-menu">
		<col width="140px " />
		<col />
		<col width="100px" />
	...
	<?php  } ?>
	<?php  if(in_array($account->typeSign, array(ACCOUNT_TYPE_SIGN, XZAPP_TYPE_SIGN))) { ?>
	<table class="table we7-table ">
		<col width="160px" />
		<col />
		<col width="230px"/>
		<tr><th class="text-left" colspan="3"><?php  echo $account->typeeName?>通信</th></tr>
		<tr>
...
<script>
	angular.module('accountApp').value('config', {
		...
		<?php  } ?>
		<?php  if($account->typeSign == XZAPP_TYPE_SIGN) { ?>
		xzapp_normal : <?php echo ACCOUNT_TYPE_XZAPP_NORMAL;?>,
		xzapp_auth : <?php echo ACCOUNT_TYPE_XZAPP_AUTH;?>,
		<?php  } ?>
		links: {
