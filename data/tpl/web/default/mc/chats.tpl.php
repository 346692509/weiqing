...
<div class="main">
	...
	<div class="notice-fans-info">
		<div class="media">
			...
			<div class="media-body">
				<h4 class="media-heading"><?php  echo $fans_info['nickname'];?></h4>
				<p>
					<span>粉丝编号：<?php  echo $fans_info['openid'];?></span>
				</p>
				<p>
					<?php  if(empty($fans_info['uid'])) { ?>
					<span>非会员</span>
					<?php  } else { ?>
					<span>会员名： <?php  echo $fans_info['member_info']['nickname'];?></span>
					<span>手机号：<?php  echo $fans_info['member_info']['mobile'];?></span>
					<span>邮箱：无</span>
					<?php  } ?>
				</p>
				<p><span>状态：<?php  if($fans_info['follow'] == 1) { ?>已关注<?php  } else { ?>取消关注<?php  } ?></span>
					<?php  if($fans_info['follow'] == 1) { ?>
	...
	<div class="notice-fans-post">
		<div class="panel we7-panel">
			<?php  echo module_build_form('core', $rid, $show_chast_content)?>
		</div>
	...
	<div class="notice-fans-list" ng-controller="chatsCtrl">
		<h2 class="notice-list-head">最近20条聊天记录</h2>
			<div class="media notice-item" ng-repeat="log in chatLogs">
				<a class="media-left" href="#">
					<img src="<?php  echo $_W['account']['logo']?>" width="48" class="head-logo" ng-if="log.flag == 1">
