...
<div id="js-user-profile" ng-controller="UserProfileDisplay" ng-cloak>
	<?php  if($_W['highest_role'] != 'founder') { ?>
	...
	<div class="btn-group we7-btn-group we7-padding-bottom">
		...
		<?php  if($_W['highest_role'] != ACCOUNT_MANAGE_NAME_CLERK) { ?>
		<?php  if($_W['highest_role'] != 'founder') { ?>
		...
		<a href="<?php  echo url('user/profile/base', array('uid' => $user['uid'], 'user_type' => PERSONAL_LIMIT_TYPE))?>" class="btn btn-default <?php  if($user_type == PERSONAL_LIMIT_TYPE) { ?>active<?php  } ?>">账号使用期限</a>
		<?php  } ?>
		<?php  } ?>
		<a href="<?php  echo url('user/profile/base', array('uid' => $user['uid'], 'user_type' => PERSONAL_BIND_TYPE))?>" class="btn btn-default <?php  if($user_type == PERSONAL_BIND_TYPE) { ?>active<?php  } ?>">账号绑定</a>
	</div>
	...
	<div class="base">
		...
		<table class="table we7-table table-hover table-form">
			...
			<tr>
				...
				<td><div class="link-group"><a href="javascript:;" data-toggle="modal" data-target="#realphone" >修改</a></div></td>
			</tr>
			<tr>
	...
	<?php  } ?>
	<?php  if($user_type == PERSONAL_LIMIT_TYPE) { ?>
		<div class="account">
			<table class="table we7-table table-hover vertical-middle">
				<tr>
					<th class="text-left">账户使用期限</th>
					<th>总有效天数:<?php  echo $total_timelimit;?></th>
					<th class="text-right">有效期至:<?php  echo $endtime;?></th>
				</tr>
				<tr>
					<td>所属用户组：<?php  echo $group_info['name'];?></td>
					<td class="text-left">有 效 天 数：<?php  if($group_info['timelimit'] == 0 || !$group_info) { ?> 永久 <?php  } else { ?> <?php  echo $group_info['timelimit'];?> <?php  } ?> </td>
					<td class="text-right"></td>
				</tr>
				<tr>
					<td>附加权限</td>
					<td class="text-left">有 效 天 数：<?php  echo $extra_limit_info['timelimit'];?> </td>
					<td class="text-right"></td>
				</tr>
			</table>
		</div>
...
<?php  if($user_type == PERSONAL_BIND_TYPE) { ?>
<!--账号绑定-->
